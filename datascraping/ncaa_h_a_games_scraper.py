...
class Game:
    ...
    url: str
    date: str
    home_won: int # 1 if home team won
...
def parse_games(html, starting_id, month, day, year):
    ...
    for game in games:
        ...
        try:
            ...
            print("error getting gametype")
        try:
            away_team = game.css_first("tr").css_first('td').text(strip=True)
        except:
            print("error getting away team")
        ...
        if loser == away_team:
            ...
            print("away_team does not match either team")
...
def to_csv(games_list, filename):
    with open(filename, "w") as f:
        writer= csv.DictWriter(f, fieldnames=["winner", "loser", "id", "url", "date", "home_won"])
